...
public class AddController {
    ...
    void submit(ActionEvent event) {
    	...
    	int[] sumPoly = new int[BasicCalculations.MAX_SIZE];
    	...
    	// calculation for adding the 2 polynomials to the summed polynomial
    	for (int index1 = (BasicCalculations.MAX_SIZE - 1); index1 >= 0; index1--) {
			sumPoly[index1] = poly1[index1] + poly2[index1]; 
		}
    	
    	String stringSumPolyFormat = null;
		...
		for (int index3 = (BasicCalculations.MAX_SIZE - 1); index3 >= 0; index3--) { 
		        if (sumPoly[index3] != 0)
		        {
		            if (max_exponent < index3)
		            {
		                ...
		                {
		                    max_exponent = index3;
		                    stringSumPolyFormat = Integer.toString(sumPoly[max_exponent]);
		                }
		            }
		            else if (sumPoly[index3] == 1)
		            {
		            	stringSumPolyFormat = stringSumPolyFormat + (" + ");
		            }
		            else if (sumPoly[index3] == -1)
		            {
		            	stringSumPolyFormat = stringSumPolyFormat + (" - ");
		            }
		            else if (sumPoly[index3] > 0)
		            {
		            	stringSumPolyFormat = stringSumPolyFormat + (" + " + sumPoly[index3]);
		            }
		            else if (sumPoly[index3] < 0)
		            {
		            	stringSumPolyFormat = stringSumPolyFormat + (" - " + (sumPoly[index3] * -1));
		            }
		            if (index3 > 1 && index3 < 21)
		            {
		            	stringSumPolyFormat = stringSumPolyFormat + ("x^" + index3);
		            }
		            if (index3 == 1)
		            {
		            	stringSumPolyFormat = stringSumPolyFormat + ("x");
		            }
		            if (index3 == 0 && sumPoly[index3] == (-1))
		            {
		            	stringSumPolyFormat = Integer.toString(sumPoly[index3] * -1);
		            }
		            if (index3 == 0 && sumPoly[index3] == 1)
		            {
		            	stringSumPolyFormat = Integer.toString(sumPoly[index3]);
		            }
		        }
		        else if (max_exponent < index3 && index3 == 0)
		        {
		        	stringSumPolyFormat = "0";
		        }
		    }	
